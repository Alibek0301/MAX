...
export const translations = {
  ru: {
    metaTitle: 'Работа в такси Астана — Таксопарк MAX | Яндекс Такси, ежедневные выплаты, низкая комиссия',
    metaDescription: 'Работа в такси Астана: присоединяйтесь к официальному партнёру Яндекс Go — таксопарк MAX. Низкая комиссия, выплаты каждый день, бесплатное подключение, поддержка 24/7.',
    metaKeywords: 'работа в такси Астана, таксопарк MAX, Яндекс Такси водитель, работа водителем Астана, подключение Яндекс Go, низкая комиссия такси, ежедневные выплаты, таксопарк Казахстан',
    home: 'Главная',
    services: 'Услуги',
    fleet: 'Автопарк',
    standards: 'Стандарты',
    ...
    heroBadge: 'MAX · Премиальный транспортный сервис в Астане',
    ...
    partnerDriversText: 'Подключаем к Яндекс.Такси за 10 минут. Устали от скрытых комиссий и задержек выплат? В парке MAX всё честно: прозрачный процент, моментальный вывод денег на любую банковскую карту и живая поддержка, которая действительно помогает, а не отвечает скриптами.',
    ...
    driverBenefit3Desc: 'Выводите заработанное на любую банковскую карту моментально, хоть каждый день — без выходных и праздников.',
  ...
  kk: {
    ...
    heroBadge: 'Астана · MAX таксопаркі · Премиум транспорт қызметі',
  ...
  en: {
    ...
    heroBadge: 'Astana · MAX taxi park · Premium transport service',
    heroTitle: 'MAX taxi park — Comfortable rides in Astana and beyond',
...
export const getDriverFaq = (language) => {
  const driverFaqByLanguage = {
    ru: [
      { q: 'Условия и комиссия?', a: 'Базовая комиссия парка — 1.9%. Для новичков действует льготная комиссия 1.7% на первые 10 дней работы. Для опытных пользователей — 1.7% комиссии через 60 дней работы в парке. Мы берём полное бухгалтерское сопровождение на себя, а также подключаем водителей к ЭДО SAPAR — вы не платите за сдачу отчётов. Если ЭДО Sapar вам не подходит вы можете написать в нашу поддержку и написать с каким сервисом электронного документооборота вам удобнее работать - мы обязательно рассмотрим ваш вопрос и при при наличи технической возможности обязательно подключимся в предложенному сервису.' },
      { q: 'Как получить выплату?', a: 'Вывод средств доступен в любое время через приложения Яндекс.Про и Alaman.app. Также вы можете собрать свою команду водителей и получать процент с каждого выполненного заказа членов команды, плюс дополнительные бонусы и призы без ограничений по времени. Обратите внимание: сам парк комиссию за вывод средств не берёт, однако удержания могут применяться со стороны платёжных сервисов при выводе.' },
      { q: 'Как брать заказы на трансфер?', a: 'Подключение к трансферам — одно из главных преимуществ парка MAX: это даёт дополнительный поток заказов и увеличивает доход. После подключения к парку новые заказы на трансфер будут доступны в телеграмм боте — уведомление «Новый заказ» нужно просто принять.' },
      { q: 'Поддержка водителей?', a: 'Поддержка водителей работает 24/7 — мы всегда на связи и готовы помочь в любое время. Также предоставляем помощь в быстром и беспроблемном подключении к парку, разбор вопросов по бухгалтерии и отчётности, а участие в конкурсах и байгах даёт дополнительные бонусы и привилегии. Дополнительно — для тарифов бизнес и выше при выполнении от 50 заказов в неделю водитель получает блок воды в подарок каждую неделю.' },
      { q: 'Открыты к предложениям?', a: 'Самое главное мы открыты к новым предложениям и идеям, вы можете написать нам свои предложения по улучшению сервиса, дополнительным привилегиям, подключению к другим услугам и сервисам, привлечению клиентов и партнеров - будьте уверены каждого предложение будет рассмотрено, и при успешной реализации вам будут предоставлены дополнительные бонусы и скидки. По контактам Максима (+7 705 570 90 91) и Алибека (+7 778 155 66 99).' }
    ],
    kk: [
      { q: 'Шарттар мен комиссия?', a: 'Парктің базалық комиссиясы — 1.9%. Жаңа бастағандар үшін алғашқы 10 күнде 1.7% жеңілдікті комиссия бар. Тәжірибелі жүргізушілер үшін паркте 60 күннен кейін 1.7% комиссия. Бухгалтерлік есепті толық өз мойнымызға аламыз және SAPAR ЭҚЖ қосамыз – есептер тапсырғаныңыз үшін төлемейсіз.' },
      { q: 'Төлемді қалай алуға болады?', a: 'Қаражатты кез келген уақытта Яндекс.Про және Alaman.app арқылы шығарып алуға болады. Парк қаражат шығару үшін комиссия алмайды.' },
      { q: 'Трансферге тапсырысты қалай алуға болады?', a: 'Трансферге қосылу қосымша тапсырыстар мен табыс әкеледі. Жан тапсырыстарды телеграм-бот ішінде «Жаңа тапсырыс» хабарламасын қабылдау арқылы ғана алуға болады.' },
      { q: 'Жүргізушілерді қолдау?', a: 'Қолдау 24/7 жұмыс істейді. Біз әрқашан байланыстамыз және кез келген уақытта көмек көрсетуге дайынбыз. Бизнес және одан жоғары тарифтерде аптасына 50 заказ орындасаңыз, тегін бір блок су аласыз.' },
      { q: 'Жаңа ұсыныстарға ашықпыз ба?', a: 'Біз жаңа ұсыныстармен идеяларға толықтай ашықпыз. Кез келген даму ұсыныстарыңызды Максим (+7 705 570 90 91) немесе Әлібекке (+7 778 155 66 99) жаза аласыз!' }
    ],
    en: [
      { q: 'Terms and commission?', a: 'The base commission is 1.9%. For beginners, there is a promo 1.7% commission for the first 10 days. Experienced users get 1.7% after 60 days. We also cover all accounting reporting and connect drivers to SAPAR electronic document flow at our cost.' },
      { q: 'How to receive payouts?', a: 'You can withdraw earnings anytime via Yandex.Pro and Alaman.app. We do not charge withdrawal fees.' },
      { q: 'How to get transfer orders?', a: 'Access to transfers provides extra income. New orders will appear in our Telegram bot — simply accept the "New Order" notification.' },
      { q: 'Driver support?', a: 'Our support operates 24/7. We assist with onboarding, accounting questions, and more. For Business class and above, completing 50 trips a week grants you a complementary pack of quality water.' },
      { q: 'Open to suggestions?', a: 'We are completely open to new ideas for improving the service, acquiring clients/partners, or connecting additional services. Contact Max (+7 705 570 90 91) or Alibek (+7 778 155 66 99) anytime.' }
    ]
  };
  return driverFaqByLanguage[language] || driverFaqByLanguage.ru;
